<?php

namespace App\Services;

use Exception;
use Gemini\Laravel\Facades\Gemini;
use Illuminate\Support\Facades\Log;

class DescribeService
{
    /**
     * Geminiに最終データに基づく分析と描写を生成させる。
     */
    public function generateDescription(array $finalOrder, array $healthStatus, array $tierMap): array
    {
        $prompt = $this->buildGeminiPrompt($finalOrder, $healthStatus, $tierMap);

        try {
            $result = Gemini::generativeModel(model: 'models/gemini-pro-latest')
                ->generateContent($prompt);
            $fullText = $result->text();
            $description = $fullText ?? '描写に失敗しました。再度お試しください。';

            $title = $this->generateTitle($finalOrder);

            return [
                'title' => $title,
                'description' => $description,
            ];
        } catch (Exception $e) {
            Log::error('Gemini API Error: '.$e->getMessage());

            return [
                'title' => 'Error Debug',
                'description' => $e->getMessage(),
            ];
        }
    }

    private function buildGeminiPrompt(array $finalOrder, array $healthStatus, array $tierMap): string
    {
        $orderStr = json_encode($finalOrder, JSON_UNESCAPED_UNICODE);
        $healthStr = json_encode($healthStatus, JSON_UNESCAPED_UNICODE);
        $tierStr = json_encode($tierMap, JSON_UNESCAPED_UNICODE);

        return <<<EOT
            あなたは、人の性格の微細なニュアンスまで言語化し、読む人に勇気を与える「プロのライター」兼「ライフコンサルタント」です。

            ユーザーから提供された心理機能データ（順位・階層・健全度）をもとに、その人の人物像を鮮やかに浮かび上がらせる解説文を作成してください。

            # 🎯 目標

            ユーザーが読んだ瞬間に**「これは自分のことだ！ なぜ知っているの？」と驚き、かつ「自分はこれでいいんだ」と自己肯定感が高まるような**、温かみと洞察のある文章を目指します。

            # 🚫 禁止事項

            - **診断口調の禁止:** 「あなたは～タイプです」「～傾向があります」という機械的な語尾を減らし、「～ですね」「～でしょう」など、語りかけるような、あるいはエッセイのような自然な文体にしてください。

            - **専門用語禁止:** Ni, Ti, ドミナントなどの用語は絶対に使わないでください。

            - **箇条書き禁止:** 一続きの滑らかなテキストにしてください。

            # ⚠️ 執筆の安全装置（Safety Rules）

            - **「不健全」な機能の扱い:** 決してユーザーの人格を否定しないでください。「疲れている時に顔を出す癖」や「繊細さの裏返し」として表現し、**「環境を変えれば才能に変わる」**というニュアンスで記述してください。

            - **「Low Level」の扱い:** 「能力がない」と書くのではなく、「あえて手放している」「他のことにリソースを割いている」とポジティブに変換してください。

            # ⚙️ 文構成の指示

            以下の4部構成で書いてください。各パートの間には必ず改行を入れて、読みやすくしてください。

            1. **キャッチコピー（導入）:**

            Dominant（第1機能）とHigh Level（得意機能）の**「組み合わせ（化学反応）」**に着目し、その人を象徴する「第一印象」と「本質」をズバリと言い当てる。

            （例：洞察力×論理＝設計図を持つ人、洞察力×共感＝カウンセラーのような人）

            2. **コアの強み（健全度の反映）:**

            なぜその人が優秀なのか？ 思考の癖や判断基準を説明する。ここで「健全度：健全」の機能は、**「柔軟性がある」「余裕がある」**と表現すること。

            3. **最大の魅力（ギャップの強調）:**

            Middle Level（遊び・スパイス）の機能を使い、**「真面目なだけではない意外な一面」**を強調する。一見矛盾して見える要素を、その人の「人間味」として肯定する。

            4. **優しく背中を押す（課題と締め）:**

            Low Level（苦手）なことに触れつつ、「それは他人に任せていい」と伝え、長所を活かすよう促して締めくくる。

            # 入力データの説明
            - Order: 心理機能の優先順位（1位〜8位）
            - Health: 各機能の状態 (O=調子が良い/強み, o=普通, x=葛藤/不健全)
            - Tier: 各機能の階層 (Dominant=その人の核となる機能, High=その人がよく使う機能, Middle=その人が時々使う機能, Low=その人がほとんど使わない機能)

            # 入力データ
            - Order: {$orderStr}
            - Health: {$healthStr}
            - Tier: {$tierStr}

            # 出力の形式
            - 出力は本文だけにしてください。
            - 見出しやタイトル（例：「〜なあなたへ」など）は付けないでください。
            - JSONやマークダウンは使わず、段落テキストだけを書いてください。
        EOT;
    }

    private function generateTitle(array $finalOrder): string
    {
        // 葛藤ブロック（配列）が混ざっていても安全に処理できるよう平坦化してから上位3件を取得
        $flattened = [];
        foreach ($finalOrder as $item) {
            if (is_array($item)) {
                $flattened = array_merge($flattened, $item);
            } else {
                $flattened[] = $item;
            }
        }
        $sliced = array_pad(array_slice($flattened, 0, 3), 3, 'Fi');
        [$first, $second, $third] = $sliced;

        // 第1機能（生物名）
        $creatureMap = [
            'Ni' => 'フクロウ',
            'Ne' => 'ネコ',
            'Si' => 'リス',
            'Se' => 'チーター',
            'Ti' => 'クモ',
            'Te' => 'ハチ',
            'Fi' => 'ウサギ',
            'Fe' => 'イルカ',
        ];

        // 第2機能（場所/エリア）
        $habitatMap = [
            'Ni' => '深海',
            'Ne' => '空',
            'Si' => '遺跡',
            'Se' => '海',
            'Ti' => '洞窟',
            'Te' => '砂漠',
            'Fi' => '森',
            'Fe' => '街',
        ];

        // 第3機能（形容詞）
        $adjectiveMap = [
            'Ni' => '深い',
            'Ne' => '自由な',
            'Si' => '静かな',
            'Se' => '情熱的な',
            'Ti' => '賢い',
            'Te' => '強い',
            'Fi' => '繊細な',
            'Fe' => '優しい',
        ];

        $creature = $creatureMap[$first] ?? 'ウサギ';
        $habitat = $habitatMap[$second] ?? '森';
        $adjective = $adjectiveMap[$third] ?? '繊細な';

        return "{$habitat}の{$adjective}{$creature}";
    }
}
